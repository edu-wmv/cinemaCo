...
export default function CinemasHeader() {
  ...
  return (
    ...
    <View
      style={{ width, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 15, position: 'absolute', paddingTop: (barH + 20), paddingBottom: 10, backgroundColor: Colors.top_gradient }}>
      <View style={{ alignItems: 'flex-start' }}>
        <Text style={{ color: Colors.text, fontSize: 32, fontFamily: 'Montserrat_Bold'}}>Cinemas</Text>
        <Pressable style={{ flexDirection: 'row', alignItems: 'baseline', paddingLeft: 4, marginTop: 2}} onPress={() => {setModalVisible(true); getStates();}}>
          <Fontisto name='map-marker-alt' color={Colors.tint} size={12} style={{ lineHeight: 12 }} />
          <Text style={{ fontFamily: 'SF_Pro_Medium', color: Colors.text_gray, fontSize: 13, marginLeft: 5, lineHeight: 13 }}>{city}</Text>
        </Pressable>
      </View>
