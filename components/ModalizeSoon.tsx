...
export default function ModalizeSoon() {
  ...
  const MovieItem = ({poster, id, title}: { poster: string, id: string, title: string }) => {
    ...
    return (
      <TouchableOpacity style={{ width: POSTER_WIDTH, paddingBottom: 12 }} activeOpacity={.65} onPress={() => { dispatch(setSoonModalOpen(false)); router.push('movie_info'); router.setParams({ id }); }}>
        ...
        <Text style={{ fontFamily: 'Montserrat_Semibold', color: Colors.text_gray_lightest, fontSize: 11 }} numberOfLines={3}>{title}</Text>
      </TouchableOpacity>
    )
  };
